...
from app.domains.files import eps_service, version_service
...
def convert_image(
    ...
    *,
    ...
    src_is_eps = src_path.suffix.lower() == ".eps"

    # EPS → EPS: keep the vector source byte-for-byte. Round-tripping through
    # Pillow's Level-2 encoder would raster-wrap the file and silently
    # discard vector content.
    if src_is_eps and pil_format == "EPS":
    ...
    # For any EPS → raster target, rasterize via Ghostscript at print-quality
    # resolution before handing off to Pillow. Bypasses Pillow's coarse
    # `scale` multiplier which under-resolves small EPS files.
    render_tmp: Path | None = None
    try:
        if src_is_eps:
            with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
                render_tmp = Path(tmp.name)
            eps_service.eps_to_png(src_path, render_tmp, target_px_edge=3000)
            open_target: Path = render_tmp
        else:
        ...
        with Image.open(open_target) as img:
            ...
            # JPEG and EPS (Pillow's Level-2 raster encoder) can't hold alpha;
            # flatten onto white so the result matches a Save As from an
            # image editor. EPS→EPS never reaches here (handled by the
            # byte-copy branch above), so a pil_format=="EPS" target here
            # implies a raster source being converted to EPS.
            ...
            try:
                if pil_format == "JPEG":
                    ...
                    raise RuntimeError(f"Unhandled target format: {pil_format}")
            except (OSError, UnidentifiedImageError) as e:
                raise RuntimeError(f"Failed to encode target: {e}") from e
    finally:
        if render_tmp is not None and render_tmp.exists():
            try:
                render_tmp.unlink()
            except OSError:
                pass
