...
export function EditorPage() {
  ...
  useDocumentTitle(`Editor — S4 Carlisle CMS`);
    ...
    <main className="page-enter px-6 py-8 max-w-3xl mx-auto w-full">
      <div className="bg-white border border-surface-200 rounded-lg p-8 shadow-sm text-center">
        <div className="w-12 h-12 bg-gold-100 rounded-full flex items-center justify-center mx-auto mb-4">
          <BookOpen className="w-6 h-6 text-gold-600" aria-hidden="true" />
        </div>
        <h1 className="text-xl font-semibold text-navy-900 mb-2">Editor</h1>
        <p className="text-sm text-navy-500 mb-6">
          The document editor for project {projectId} will be available here.
        </p>
        <Link
          to={uiPaths.projectDetail(projectId ?? "")}
