    ...
    <div className="bg-white rounded-lg shadow-card border border-navy-100 flex flex-col h-full">
      ...
      <div className="p-3 border-b border-navy-100">
        <select
          ...
          className="w-full text-sm border border-navy-200 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        >
            ...
            {filteredRules.map((rule: IARow, idx: number) => (
              <div key={idx}>
                <button
                  ...
                  className="w-full text-left px-3 py-2 rounded-md bg-surface-50 border border-navy-100 hover:bg-navy-100 transition-all"
                >
                  <div className="flex items-center gap-2">
                    <ChevronDown
                      className={`w-3 h-3 text-navy-400 transition-transform ${
                ...
                {expandedRules.has(idx) && (
                  <div className="mt-1 ml-4 p-2 bg-blue-50 border-l-2 border-blue-300 rounded text-[10px] text-navy-600 space-y-1">
                    {rule.element && (
                      <div>
      ...
      {/* Info */}
      {stylesheet && (
        <div className="border-t border-navy-100 p-3 bg-blue-50 text-[10px] text-navy-600">
          ...
          {stylesheet.is_active && (
            <p className="text-[9px] text-green-600 mt-1 font-semibold">✓ Active Stylesheet</p>
          )}
        </div>
      )}
    </div>
