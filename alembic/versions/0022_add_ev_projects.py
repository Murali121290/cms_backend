...
def upgrade() -> None:
    ...
    if inspector.has_table("epub_validator_book_events"):
        op.drop_table("epub_validator_book_events")
    if inspector.has_table("epub_validator_books"):
        op.drop_table("epub_validator_books")

    # 2. Create the new project-based table if not exists
    if not inspector.has_table("post_prod_ev_projects"):
        op.create_table(
    ...
    if not inspector.has_table("post_prod_ev_history"):
        op.create_table(
